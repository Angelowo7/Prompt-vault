...
if (!PV_IS_TOP_FRAME) {
  ...
  // Place the FAB loosely near the cursor — offset down-right so it doesn't sit
  // on top of the line being typed. Only called on FIRST appearance; after that
  // showFab keeps it put, so it doesn't chase the mouse.
  function placeFabNearCursor(mouseX, mouseY, field) {
    const size = 30;
    const offsetX = 28;   // a bit away from the caret, not glued to it
    const offsetY = 18;   // sit below-right of the cursor, clear of the text line

    let left = mouseX + offsetX;
    let top  = mouseY + offsetY;

    // If there's no room on the right, flip to the left of the cursor
    if (left + size > window.innerWidth - 8) left = mouseX - offsetX - size;
  ...
  // Show FAB. Places near cursor only the first time it appears; afterwards it
  // keeps its position and does NOT follow the mouse.
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (field) {
      ...
      activeField = field;
      lastField = field;
      // showFab places it near the cursor ONLY on first show; moving between
      // fields does NOT re-place it, so it won't keep chasing the mouse.
